      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        ...
        {Object.entries(CATEGORY_LABELS).map(([value, label]) => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
              categoryFilter === value ? 'bg-primary text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'
            }`}
          >
            {label}
          </button>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-gray-400">{CATEGORY_LABELS[ann.category]}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-4 text-xs text-gray-400">
                    ...
                    {ann.effectiveDate && (
                      <span>生效日期：{ann.effectiveDate}</span>
                    )}
